...
def visualize(
    ...
    for i in range(samples_num):
        all_gen_z = torch.randn([len(phases), G.z_dim], device=device)
        ws = G_ema.mapping(all_gen_z, c=None, truncation_psi=1.0)
        img1 = G_ema.synthesis(ws)
        save_image_grid(img1.cpu().numpy(), os.path.join(run_dir, f'fakes{i:06d}.png'), drange=[-1, 1], grid_size=(1,1))
