...
def rademacher(shape, device='cpu'):
    ...
    x = th.empty(shape, device=device)
...
def stack_var_and_reduce(list_of_activations, reduction=th.max):
    """Equation (5) from the paper."""
    second_orders = th.stack(list_of_activations)  # (k, N, C, H, W)
    var_tensor = th.var(second_orders, dim=0, unbiased=True)  # (N, C, H, W)
    penalty = reduction(var_tensor)  # (1,) (scalar)
    return penalty

def multi_stack_var_and_reduce(sdds, reduction=th.max, return_separately=False):
