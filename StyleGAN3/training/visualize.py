...
def visualize(
    ...
    for index, key in enumerate(eigvec_dict.keys()):
        for j in range(G_kwargs['diag_size']):
            imglists = []
            for i in np.linspace(-200, 200, 7):

                direction = i * eigvec_dict[key][:, j].unsqueeze(0).to(ws)
                img1 = G_ema.synthesis.forward_test(
                    ws,
                    direction,
                    index,
                    is_ortho)
            ...
            imglists = torch.cat(imglists, dim=0)
            save_image_grid(imglists.cpu().numpy(), os.path.join(run_dir, f'fakes{index:06d}_{j:02d}.png'), drange=[-1, 1], grid_size=(7, 1))
