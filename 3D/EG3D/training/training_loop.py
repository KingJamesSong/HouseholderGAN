...
def training_loop(
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        for name, module in [('G', G), ('D', D), ('G_ema', G_ema)]:
            ...
            print(
                'gamma not in resume_pkl', 'gamma' not in resume_pkl
            )

            if 'gamma' not in resume_pkl:

                if name == 'G':
                    model_state_dict = dict(resume_data[name].named_parameters())
                    for name_, layer in G.named_modules():
                        if 'affine' in name_ and 'superresolution' not in name_:
                            print(name_ + '.weight')
                            weight = model_state_dict[name_ + '.weight']
                            layer.intialize(weight)

                if name == 'G_ema':
                    model_state_dict = dict(resume_data[name].named_parameters())
                    for name_, layer in G_ema.named_modules():
                        if 'affine' in name_ and 'superresolution' not in name_:
                            print(name_ + '.weight')
                            weight = model_state_dict[name_ + '.weight']
                            layer.intialize(weight)
